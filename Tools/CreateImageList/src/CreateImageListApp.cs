...
namespace CreateImageList
{
	class CreateImageListApp
	{
		...
		public CreateImageListApp(Guid package, string moduleName, string baseDirectory)
		{
			this.guidPackage = package;
			using (RegistryKey key = Registry.LocalMachine.OpenSubKey(
				@"SOFTWARE\Microsoft\VisualStudio\8.0\Packages\" + package.ToString("b"), false))
			{
		...
		private void WriteFooter()
		{
			...
			this.ctcFile = null;
			Console.WriteLine("Procesed {0} icons for package {1}", this.imageList.Images.Count, this.moduleName);
		}
